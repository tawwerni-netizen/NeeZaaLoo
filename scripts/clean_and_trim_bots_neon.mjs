...
async function main() {
  console.log("=== Bot Cleaning and Trimming to 204 Realistic Bots ===");

  // 1. Build list of 204 bot IDs to keep
  const keepBotIds = ["ai-easy", "ai-medium", "ai-hard", "ai-expert"];

  for (let i = 1; i <= 20; i++) {
    keepBotIds.push(`bot_p_${String(i).padStart(3, "0")}`);
  }

  const langs = [
    { prefix: "ar", list: AR_NAMES, offset: 1 },
    { prefix: "en", list: EN_NAMES, offset: 15 },
    { prefix: "es", list: ES_NAMES, offset: 30 },
    { prefix: "fr", list: FR_NAMES, offset: 45 },
    { prefix: "hi", list: HI_NAMES, offset: 60 },
    { prefix: "zh", list: ZH_NAMES, offset: 75 },
  ];

  for (const { prefix } of langs) {
    for (let i = 1; i <= 30; i++) {
      keepBotIds.push(`bot_${prefix}_${String(i).padStart(3, "0")}`);
    }
  }

  console.log(`Target realistic bot count: ${keepBotIds.length} (4 single-player + 20 elite + 180 international)`);

  // 2. Find excess bots to delete
  const keepIdListStr = keepBotIds.map((id) => `'${id}'`).join(",");
  const excessBots = await queryHttp(
    `SELECT id, handle FROM player WHERE (is_ai = TRUE OR id LIKE 'bot_%' OR id LIKE 'ai-%') AND id NOT IN (${keepIdListStr});`
  );
  console.log(`Found ${excessBots.length} excess bots to remove.`);

  if (excessBots.length > 0) {
    const excessIds = excessBots.map((r) => `'${r.id}'`);
    const BATCH = 50;

    for (let i = 0; i < excessIds.length; i += BATCH) {
      const batchIds = excessIds.slice(i, i + BATCH).join(",");
      const cleanupStmts = [
        `DELETE FROM rating WHERE player_id IN (${batchIds});`,
        `DELETE FROM tournament_registration WHERE player_id IN (${batchIds});`,
        `DELETE FROM friendship WHERE user_id IN (${batchIds}) OR friend_id IN (${batchIds});`,
        `DELETE FROM direct_message WHERE sender_id IN (${batchIds}) OR receiver_id IN (${batchIds});`,
        `UPDATE player SET
           handle = 'trimmed_' || id,
           is_ai = FALSE,
           disabled_at = now(),
      ...
      await execHttp(cleanupStmts);
      console.log(`  ✓ Trimmed batch ${i + 1} to ${Math.min(i + BATCH, excessIds.length)} excess bots`);
    }
  }

  // 3. Update the 204 kept bots with authentic human profiles, real portraits, and bios
  console.log("\nUpdating kept bots with realistic human profiles, real portraits, and bios...");

  // Reset handles to temporary values to prevent any unique constraint collision during renaming
  await execHttp([`UPDATE player SET handle = 'tmp_' || id WHERE is_ai = TRUE;`]);
  console.log("  ✓ Reset bot handles to temporary values to avoid collisions");

  // Single player bots
  const singlePlayerStmts = [
    `UPDATE player SET handle = 'EasyBot', bio = 'لاعب شطرنج هاوٍ يتدرب على التكتيكات الأساسية ♟️', avatar_key = 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150&auto=format&fit=crop&q=80', allow_direct_messages = FALSE, is_ai = TRUE WHERE id = 'ai-easy';`,
    `UPDATE player SET handle = 'MediumBot', bio = 'منافس متوازن يتقن افتتاحيات الشطرنج وألعاب الطاولة 🎲', avatar_key = 'https://images.unsplash.com/photo-1570295999919-56ceb5ecca61?w=150&auto=format&fit=crop&q=80', allow_direct_messages = FALSE, is_ai = TRUE WHERE id = 'ai-medium';`,
    `UPDATE player SET handle = 'HardBot', bio = 'محترف استراتيجيات وألعاب لوحية، تصنيف متقدم ⚡', avatar_key = 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80', allow_direct_messages = FALSE, is_ai = TRUE WHERE id = 'ai-hard';`,
    `UPDATE player SET handle = 'GrandmasterAI', bio = 'جراند ماستر في التكتيكات والحسابات العميقة 👑', avatar_key = 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=150&auto=format&fit=crop&q=80', allow_direct_messages = FALSE, is_ai = TRUE WHERE id = 'ai-expert';`,
  ];
  await execHttp(singlePlayerStmts);
  console.log("  ✓ Updated 4 single-player bots");

  // 20 Elite High Rollers
  const eliteStmts = [];
  for (let i = 0; i < 20; i++) {
    ...
    const safeBio = item.bio.replace(/'/g, "''");
    const safeHandle = item.handle.replace(/'/g, "''");

    eliteStmts.push(
      `UPDATE player SET handle = '${safeHandle}', bio = '${safeBio}', avatar_key = '${avatarUrl}', allow_direct_messages = FALSE, is_ai = TRUE WHERE id = '${botId}';`
    );
  }
  await execHttp(eliteStmts);
  console.log("  ✓ Updated 20 elite bots with realistic portraits and bios");

  // 180 International bots
  for (const { prefix, list, offset } of langs) {
    const langStmts = [];
    for (let i = 0; i < 30; i++) {
    ...
    await execHttp(langStmts);
    console.log(`  ✓ Updated 30 ${prefix} bots with authentic human portraits and bios`);
  }

  // 4. Seed realistic ELO ratings for the 204 bots
  console.log("\nSeeding realistic ELO ratings for all 204 bots...");
  const games = ["chess", "xo", "connect-four"];
  const ratingStmts = [];

  for (const botId of keepBotIds) {
    for (const game of games) {
      // Gaussian distribution around 1400-2100 ELO
      let u = 0, v = 0;
      ...
      const rating = Math.floor(900 + num * 1800);
      const rating_x100 = rating * 100;
      const rd_x100 = 3000 + Math.floor(Math.random() * 4000);
      const games_played = Math.floor(20 + Math.random() * 250);

      ratingStmts.push(
        `INSERT INTO rating (player_id, game_id, rating_x100, rd_x100, games_played)
         VALUES ('${botId}', '${game}', ${rating_x100}, ${rd_x100}, ${games_played})
  ...
  const RATING_BATCH = 50;
  for (let i = 0; i < ratingStmts.length; i += RATING_BATCH) {
    const batch = ratingStmts.slice(i, i + RATING_BATCH);
    await execHttp(batch);
  }
  console.log(`  ✓ Seeded realistic ratings across ${games.join(", ")}`);

  // 5. Verification
  const countRow = await queryHttp("SELECT count(*) as count FROM player WHERE is_ai = TRUE;");
  const sampleBots = await queryHttp(
    "SELECT id, handle, bio, avatar_key FROM player WHERE is_ai = TRUE ORDER BY id LIMIT 10;"
  );

  console.log("\n=== Final Verification ===");
  console.log(`Total verified bots: ${countRow[0]?.count} (Expected exactly 204)`);
  console.log("\nSample realistic bots:");
  for (const b of sampleBots) {
    console.log(`- [${b.id}] ${b.handle} | Avatar: ${b.avatar_key?.slice(0, 40)}... | Bio: ${b.bio?.slice(0, 50)}`);
  }
}

main().catch((err) => {
  console.error("Bot cleaning failed:", err);
  process.exit(1);
});
