...
const connectionString = "postgresql://neondb_owner:npg_v43zqGfVSXnM@ep-rapid-cell-b1108r3p-pooler.c-5.eu-central-1.aws.neon.tech/neondb?channel_binding=require&sslmode=require";
...
async function main() {
  ...
  try {
    ...
    // 3. Trim all excess bots from database
    const excessBots = await client.query(
      `SELECT id FROM player WHERE is_ai = TRUE AND id != ALL($1)`,
      [keepBotIds]
    );
    console.log(`Found ${excessBots.rows.length} excess bots to remove.`);
    ...
    if (excessBots.rows.length > 0) {
      ...
      console.log("Cleaned ratings for excess bots.");

      // Clean tournament registrations
      await client.query(`DELETE FROM tournament_registration WHERE player_id = ANY($1)`, [excessIds]);
      console.log("Cleaned tournament registrations for excess bots.");

      // Attempt to delete excess bots from player table, or deactivate if referenced by past duels
      try {
        await client.query(`DELETE FROM player WHERE id = ANY($1)`, [excessIds]);
        console.log(`Deleted ${excessIds.length} excess bots from player table.`);
      } catch (err) {
        console.log("Some excess bots are referenced in past historical records, deactivating them and releasing their handles...");
        await client.query(`
          UPDATE player SET
            handle = 'trimmed_' || id,
            is_ai = FALSE,
          ...
          WHERE id = ANY($1)
        `, [excessIds]);
        console.log(`Deactivated ${excessIds.length} excess bots (is_ai = FALSE, disabled, handles freed).`);
      }
    }

    // 4. Update the 204 kept bots with realistic human portraits, authentic handles, and disable DMs
    console.log("Updating kept bots with realistic avatars and authentic profiles...");

    // Update single-player AI bots
    await client.query(`
      UPDATE player SET
        handle = CASE id
          WHEN 'ai-easy' THEN 'EasyBot'
          WHEN 'ai-medium' THEN 'MediumBot'
          WHEN 'ai-hard' THEN 'HardBot'
          WHEN 'ai-expert' THEN 'GrandmasterAI'
          ELSE handle END,
        avatar_key = CASE id
          WHEN 'ai-easy' THEN 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150&auto=format&fit=crop&q=80'
          WHEN 'ai-medium' THEN 'https://images.unsplash.com/photo-1570295999919-56ceb5ecca61?w=150&auto=format&fit=crop&q=80'
          WHEN 'ai-hard' THEN 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80'
          WHEN 'ai-expert' THEN 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=150&auto=format&fit=crop&q=80'
          ELSE avatar_key END,
        allow_direct_messages = FALSE,
        is_ai = TRUE
      WHERE id IN ('ai-easy', 'ai-medium', 'ai-hard', 'ai-expert')
    `);

    // Update 20 elite bots
    for (let i = 0; i < 20; i++) {
    ...
    // Helper for language bots
    async function updateLangBots(prefix, list, offset) {
      for (let i = 0; i < 30; i++) {
    ...
    await updateLangBots("ar", AR_NAMES, 1);
    await updateLangBots("en", EN_NAMES, 15);
    await updateLangBots("es", ES_NAMES, 30);
    await updateLangBots("fr", FR_NAMES, 45);
    await updateLangBots("hi", HI_NAMES, 60);
    await updateLangBots("zh", ZH_NAMES, 75);

    console.log("All 204 bots updated successfully with realistic avatars and authentic profiles!");

    // Verification count
    const finalCount = await client.query("SELECT count(*) FROM player WHERE is_ai = TRUE");
    console.log(`Final verified bot count in database: ${finalCount.rows[0].count}`);
