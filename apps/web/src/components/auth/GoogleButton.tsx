...
export function GoogleButton({ onError, className, label, returnTo }: GoogleButtonProps) {
  ...
  async function onClick() {
    ...
    try {
      ...
      if (isUnconfigured) {
        onError?.(
          locale === "ar"
            ? "تسجيل الدخول المباشر بـ Google يتطلب ربط مفاتيح OAuth في السيرفر. يمكنك الدخول فوراً ببريد Gmail بدون كلمة مرور عبر رمز التحقق السريع أدناه:"
            : "Google OAuth is not configured yet. You can sign in immediately with your Gmail address using the instant email code button below:"
        );
      } else {
  ...
  return (
    <div className={styles.googleWrap}>
      ...
      {showFallback && (
        <LocaleLink href="/login/code" className={styles.googleFallbackBtn}>
          <span>📩</span>
          <span>{locale === "ar" ? "الدخول الفوري برمز التحقق (Gmail) بدون كلمة مرور" : "Instant Gmail Login via Verification Code"}</span>
        </LocaleLink>
      )}
    </div>
  );
