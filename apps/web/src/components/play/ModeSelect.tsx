 ...
 * Reads ONLY the plugin's own declared capabilities (supportsAI) to decide
 * which cards are offered -- never a gameId switch. TOURNAMENT deliberately
 * never opens an inline flow here: the tournament engine is its own,
 * already-complete system (packages/tournament), so this card is a link
 * into the existing tournaments surface, not a duplicate of it.
 */
...
export type PlayMode = "VS_COMPUTER" | "FRIEND" | "COMPETITIVE";
  ...
  return (
    <div>
      ...
      <div className={styles.grid}>
        ...
        <button className={styles.card} onClick={() => onSelect("COMPETITIVE")}>
          <span className={styles.cardTitle}>{t("play.mode.competitive.title")}</span>
          <span className={styles.cardDescription}>{t("play.mode.competitive.description")}</span>
        </button>
